...
function mergeLogs(existing, incoming) {
  ...
  for (const log of existing || []) {
    if (log?.id && log?.kind) merged.push(log);
  }
  for (const log of incoming || []) {
    if (!log?.id || !log?.kind) continue;
    for (let i = merged.length - 1; i >= 0; i -= 1) {
      if (sameUploadRecord(merged[i], log)) merged.splice(i, 1);
    }
    merged.push(log);
  }
...
async function getRepoInfo({ refresh = false } = {}) {
  if (!refresh && repoInfoCache && Date.now() - repoInfoCacheAt < REPO_CACHE_MS) return repoInfoCache;

  const response = await githubFetch(repoPrefix(), { cache: 'no-store' });
  ...
  repoInfoCache = await response.json();
  repoInfoCacheAt = Date.now();
  return repoInfoCache;
}

async function assertPrivateRepo(options = {}) {
  const repo = await getRepoInfo(options);
  if (!repo.private) {
...
async function readRepoFile(path) {
  const response = await githubFetch(
    `${contentsPath(path)}?ref=${encodeURIComponent(CONFIG.githubBranch)}&t=${Date.now()}`,
    { cache: 'no-store' },
  );
...
async function listEventFiles() {
  ...
  const response = await githubFetch(
    `${contentsPath(root)}?ref=${encodeURIComponent(CONFIG.githubBranch)}&t=${Date.now()}`,
    { cache: 'no-store' },
  );
...
async function writeRepoFile(path, text, { sha = null, message }) {
  ...
  let response;
  try {
    response = await githubFetch(contentsPath(path), {
      method: 'PUT',
      cache: 'no-store',
    ...
    const error = new Error(`GitHub could not save ${path}: ${cause.message || cause}.`);
    error.code = 'GITHUB_WRITE_TRANSIENT';
    error.cause = cause;
    throw error;
  }
  ...
  if (!response.ok) {
    ...
    if (response.status === 409 || response.status === 422) error.code = 'GITHUB_WRITE_CONFLICT';
    else if ([500, 502, 503, 504].includes(response.status)) error.code = 'GITHUB_WRITE_TRANSIENT';
...
async function getAllLogs({ onLatest } = {}) {
  ...
  const older = await Promise.all(files.slice(1).map(({ path }) => readPeriod(path)));
  return mergeLogs(latestLogs, older.flatMap((file) => file.logs));
}
...
async function savePeriod(path, newLogs) {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    try {
      ...
      lastError = error;
      const retryable = error.code === 'GITHUB_WRITE_CONFLICT' || error.code === 'GITHUB_WRITE_TRANSIENT';
...
async function saveLogs(logs) {
  ...
  for (const [path, newLogs] of groups) {
    written += await savePeriod(path, newLogs);
  }
  return written;
}
